    ...
    (set, get) => ({
      ...
      setClaudeCodeStatus: (status) => {
        set((state) => ({
          agentStatuses: {
            ...state.agentStatuses,
            'claude-code': status ? { ...status, lastChecked: Date.now() } : null,
          },
          isCheckingAgent: false,
        }))
      },
 ...
 */
export function isWorkspaceAgentReady(state: SettingsState, agentId: AgentId): boolean {
  // Cloud models are always ready
 ...
 */
export function getCurrentAgentStatus(state: SettingsState): AgentStatus | null {
 ...
 */
export function getAgentStatus(state: SettingsState, agentId: AgentId): AgentStatus | null {
 ...
 * @deprecated Use isWorkspaceAgentReady instead
 */
export function isSpecificAgentReady(state: SettingsState, agentId: AgentId): boolean {
