...
export function MarketplacePage() {
  ...
  const fetchSkills = async () => {
    setIsLoading(true)
    try {
      // Fetch from our backend proxy (handles CORS and normalizes response)
      const response = await fetch(`${API_URL}/api/skillsmp/skills`)
      if (response.ok) {
        const data = await response.json()
        const fetchedSkills = data.skills || []
        setSkills(fetchedSkills.length > 0 ? fetchedSkills : DEMO_SKILLS)
      } else {
    ...
    <div className="h-full overflow-auto bg-neutral-950">
      ...
      <section className="relative min-h-[25vh] flex flex-col justify-center px-6 md:px-12 pt-8">
        <div className="flex items-end justify-between">
          ...
          <span className="hidden md:block font-mono text-sm text-neutral-600">
            ( _{String(skills.length).padStart(2, '0')} )
          </span>
        </div>
      </section>
